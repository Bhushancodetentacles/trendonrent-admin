...
const Calender = props => {
  ...
  let [productName, setProductName] = useState(location?.state?.item?.name)
  ...
  const calenderAvailability = async () => {
    ...
    try {
      if (location?.state?.item?.uniqueCode) {
        ...
        setCalender(result)
        console.log("setCalender---", result)
      }
    } catch (error) {
      console.log(error)
    }
    // try {
    //   // const res = await get(
    //   //   `Order/getCalendarAvailability?uniqueCode=${uniqueCode}`
    //   // )
    //   // const result = await res.data.list
    //   // console.log(result)
    //   // result.forEach(element => {
  ...
  // to get the data from product unique id start
  let [filterDataName, setFilterDataName] = useState("")
  const getDataUsingUniqueCode = async () => {
    console.log("hiii")
    try {
      const response = await get(
        `Product/viewByUniqueCode?uniqueCode=${uniqueCode}`
      )
      if (response) {
        setFilterDataName(response.data.result.name)
      }
  ...
  // to get the data from product unique id end

  useEffect(() => {
    getDataUsingUniqueCode()
  ...
  const proAvailabilityCalender = async () => {
    console.log("uniqueCode")
    console.log(uniqueCode)
    try {
      const res = await get(
        `Order/getCalendarAvailability?uniqueCode=${uniqueCode ? uniqueCode : location?.state?.item?.uniqueCode}`
      )
      const result = await res.data.list
      console.log(result)
      result.forEach(element => {
  ...
  useEffect(() => {
    let element = document.getElementById("calender_tab")
    if (element) {
          ...
          <Row>
            <Col className="col-12">
                ...
                <div className="card-body">
                  ...
                  >
                    <div className="col-lg-6 col-xl-3">
                      <label className="form-label header">Product Name</label>
                      {/* <p className="text-muted mb-0">{filterDataName ? filterDataName : productName ? productName:""}</p> */}
                      {/* <h5>{uniqueCode ? uniqueCode : location?.state?.item?.uniqueCode}</h5> */}
                      <input
                        type="text"
                    ...
                    <div className="col-lg-6 col-xl-3">
                      ...
                      <input
                        ...
                        value={
                          uniqueCode
                            ? uniqueCode
                            : location?.state?.item?.uniqueCode
                        }
                      />
                    </div>
                    ...
                    <div
                      ...
                      onClick={proAvailabilityCalender}
                    >
                      <div className="mt-0 mt-lg-4 mb-xxl-0 mt-sm-4 mt-xs-4">
              ...
              <Row className="mx-auto">
                ...
                <div className="card">
                  <div className="card-body">
                    <Col lg={10} className="mx-auto justify-center">
